<?php
    ...
    $query = "select cle_card,nbr,`info`,`ALL_CARDS`.`Nom`,`ALL_CARDS`.`Type`,`ALL_CARDS`.`carte`,`ALL_CARDS`.`extension` from `MARKET` inner join `ALL_CARDS` on `cle_card`=`ALL_CARDS`.`cle` where `cle_user`=(select `cle` from `ALL_USERS` where `pseudo`='".$pseudo."')";
